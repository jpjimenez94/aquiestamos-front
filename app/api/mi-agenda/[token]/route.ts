...
/**
 * Proxy público hacia la agenda de la persona.
 *
 * Sin sesión a propósito: quien abre este enlace no tiene cuenta en el portal,
 * solo su enlace de WhatsApp. El backend valida la firma del token y decide
 * qué mostrar; esto solo transporta.
 *
 * Reenvía la IP del cliente porque el backend limita peticiones por IP y sin
 * esto vería siempre la de Vercel — el límite se repartiría entre todas las
 * personas a la vez.
 */
...
async function reenviar(request: Request, token: string, metodo: 'GET' | 'POST') {
  const destino = `${BACKEND_URL}/api/mi-agenda/${encodeURIComponent(token)}`
  ...
  try {
    ...
    console.error('[mi-agenda] backend inalcanzable:', error)

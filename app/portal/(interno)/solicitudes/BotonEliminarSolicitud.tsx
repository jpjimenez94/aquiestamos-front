...
export function BotonEliminarSolicitud({
  ...
  async function eliminar() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/portal/support-requests/${solicitudId}`, {
        method: 'DELETE',
      })
      const datos = await res.json()
      if (!res.ok || !datos.success) {
        setError(datos.message ?? 'No se pudo eliminar la solicitud')
        setCargando(false)
        return
      }
  ...
  return (
    <div className="modal-eliminar-overlay" onClick={() => !cargando && setConfirmando(false)}>
      ...
      >
        ...
        <h3 id="modal-eliminar-titulo" className="modal-eliminar__titulo">
          ¿Eliminar esta solicitud?
        </h3>
        <p className="modal-eliminar__cuerpo">
          Estás a punto de eliminar la solicitud de{' '}
          <strong>{nombrePersona}</strong>. Esta acción aplica un borrado lógico
          —el registro queda en la base de datos para fines de auditoría— pero
          dejará de aparecer en el portal.
        </p>
        {error && (
